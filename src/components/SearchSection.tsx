  ...
  const calculateRankInCategory = async (studentGrade: number, category: string) => {
    try {
      ...
      if (!isConnected) {
        ...
        const categoryResults = sampleResults.filter(r => r.category === category);
        const betterGrades = categoryResults.filter(r => r.grade > studentGrade);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {connectionError && (
            <div className={`mb-6 p-4 rounded-xl border-2 text-center ${
              ...
              <p className="text-sm">
                يتم البحث في البيانات المحفوظة محلياً - النتائج الكاملة ستظهر عند توفر الاتصال
              </p>
            </div>
          )}
